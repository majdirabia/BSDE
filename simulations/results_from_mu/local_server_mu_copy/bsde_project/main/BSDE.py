...
class BsdeHD(object):
    ...
    def Z_computation(self, j, X, Y, dB, dt, rf, t):
        dB_j = dB[t, :, j]
        rf.fit(X, Y * dB_j)
    ...
    def get_price(self, N, m, RF_n_estimators=100, RF_max_leaf_nodes=50,
                  ...
                  RF_min_samples_leaf=1,
                  RF_warm_start=False,
                  option_payoff='geometric', oType='European', n_picard=5,
                  regression='RF'):
        ...
        if (oType == 'European'):
            ...
            for t in range(m - 1, 0, -1):
                ...
                if self.r != self.R:
                    # Regression for Z
                    import parmap
                    z_int = parmap.map(self.Z_computation, list(range(self.p)), X, Y, dB, dt, rf, t)
