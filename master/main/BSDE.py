...
class BSDE(object):
    ...
    def get_price_lsm(self, R, r, N, m, K1=95., K2=105., deg=8, oPayoff="call", oType=OptionPayoff.EUROPEAN,
        ...
        :param r: float -> borrow rate
        :param N: int -> #Monte Carlo simulation
        :param m: int -> discretization factor
        :param K1: float -> strike 1
        :param K2: float -> strike 2
        :param deg: int  -> polynomial degree used for regression
        :param oPayoff: string -> option payoff, i.e. call or put
        :param oType: string -> option type : European or American
        :param n_picard: int -> Picard iteration number
        :return: Option Price
        """
        ...
        if (oType == OptionPayoff.EUROPEAN):
            ...
            for t in range(m - 1, 0, -1):
                ...
                if n_picard > 0:
                    for __ in range(n_picard):
                        reg = np.polyfit(X,
                                         Y - theta * Z * dt - np.minimum(Y - (1. / self.sigma) * Z, 0) * (R - r) * dt,
                                         deg)
    ...
    def get_price_mesh(self, R, r, N, m,
        ...
        def do_BSDE_mesh_update(Y, Y_inc, Z, dB, W, cv, cv_expectation, mode='all', use_variance_reduction=False):
            """
...
class BsdeHD(object):
    ...
    def generate_correlated_paths(self, m):
        ...
        S[0] = self.S_init
        self.Cholesky = np.linalg.cholesky(self.M)
        for t in range(1, m + 1):
            rand = np.random.standard_normal(self.p)
            rand_int = np.dot(self.Cholesky, rand)
    ...
    def exercise_matrix(self, S, N, m, oType, oPayoff, K1=95., K2=105.):
        ...
        if (oPayoff == 'geometric'):
            ...
            if self.p % 2 == 0:
                ...
                for j in range(N):
                    for t in range(m + 1):
                        S1 = np.prod(S[t, j, :k]) ** (1. / k)
                        S2 = np.prod(S[t, j, k:]) ** (1. / k)
                        pay_int = S1 - S2
                        pay = max(pay_int, 0)
                        payoff[t, j] = pay
    ...
    def get_price_lsm(self, R, r, N, m, K1=95., K2=105.,
                      option_type='call', option_payoff='geometric',
                      oType='European', n_picard=10, deg=5):
        ...
        if self.p == 1:
            ...
            if (oType == 'European'):
                ...
                for t in range(m - 1, 0, -1):
                    ...
                    Z = np.zeros([N, self.p])
                    Z_sum = np.zeros(N)
                    if self.r != self.R or self.r != self.mu:
                        # Regression for Z

                        dB_k = dB[t, :, 0]
                        reg_Z = np.polyfit(X, Y * dB_k, deg)
                        Z = np.polyval(reg_Z, (X) * (1. / dt))
                        reg_Y = np.polyfit(X, Y, deg)
                        J = np.polyval(reg_Y, X)
                        Y_new = df * (J - (self.mu - self.r) * Z * dt -
                                      np.minimum(Y - Z, 0) *
                                      (self.R - self.r) * dt)
                    else:
                        reg = np.polyfit(X, Y, deg)
            ...
            raise Exception("No lsm for high dimension")
                ...
                if n_picard > 0:
                    for __ in range(n_picard):
                        ...
                        for k in range(self.p):
                            ...
                            y = NN_k[1]
                            grad_W_k = scipy.sparse.lil_matrix((N, N))
                            grad_W_k[x, y] = - (X_k[y] - X_k[x]) / l ** 2 * np.exp(
                                - (X_k[y] - X_k[x]) ** 2 / (2 * l ** 2))
                            marg_grad_W_k = scipy.sparse.diags(np.array(grad_W_k.sum(axis=0))[0])
                            grad_k = (grad_W_k * scipy.sparse.diags(
                                np.array(W_marg)[0]) - W * marg_grad_W_k) / np.square(W_marg)
    ...
    def get_price(self, N, m, RF_n_estimators=100, RF_max_leaf_nodes=50,
        ...
        Sigma = self.Cholesky * self.sigma
        try:
        ...
        for t in range(m - 1, 0, -1):
            ...
            if self.r != self.R or self.r != self.mu:
                ...
                Z = np.sum(np.dot(np.array(z_int).T, Sigma_inv), axis=1)
                ...
                for __ in range(n_picard):
                    z_int = parmap.map(Z_computation, list(range(self.p)), X, Y - Y_new, dB, dt, rf, t,
                                       processes=n_threads)
    ...
    def get_price_mesh(self, N, m,
        ...
        if mode == 'all':
            if (oType == 'European'):
                ...
                # volatility = sigma
                # weight matrix
                W = np.zeros([self.p, N, N])
                transition_3d_matrix = np.zeros([self.p, N, N])
                marginal_matrix = np.zeros([self.p, N])
                ...
                for t in range(m - 1, 0, -1):
                    ...
                    if display_plot:
                        ...
                        plt.title("Marginal Density at time {}".format(str(t * dt)))
                    W_joint = np.prod(transition_3d_matrix, axis=0)  # product over assets
                    marginal_vector = np.mean(W_joint, axis=0)  # sum product over j's
                    # marginal_matrix = np.mean(transition_3d_matrix, axis=1)
                    # weight matrix
                    W_joint = W_joint / marginal_vector
                    # W = transition_3d_matrix / marginal_matrix[:, :, np.newaxis]
                    # normalize the rows
...
class Touzi(object):
    ...
    def portfolio_opt(self, eta, eps, N, RF_n_estimators=100, RF_max_leaf_nodes=50, M=40):
        ...
        for t in range((self.m - 1), 0, -1):
            ...
            for j in range(N):
                f = lambda theta: - (0.5 * theta ** 2 * max(X[j, 1], eps) * Gamma_1_1[j] + theta * self.mu * Z_1[j])
                # for theta in np.linspace(eps, M, 10):
                #     print("value of the function a time {} for particle {} with theta {} is {}".format(t, j, theta,
                #                                                                                        f(theta)))
                opt_theta = scipy.optimize.minimize_scalar(f, bounds=(eps, M), method='bounded')
